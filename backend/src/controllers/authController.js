...
export const registerTenant = async (req, res) => {
  ...
  try {
    ...
    // ✅ AUDIT LOGGING
    // We log after commit to ensure it's only logged if successful. 
    // Note: auditLog uses its own connection/transaction logic (single query), 
    // or we could pass the client if we wanted it in the same transaction.
    // implementation of auditLog uses pool.query which gets a new client. 
    // Since we just committed, this is fine.
    try {
      await auditLog({
        tenantId,
      ...
      // Don't fail the request if logging fails, but maybe alert admin
    }
